...
#ifndef ORBITAL_MOUSE_RESET_TIMEOUT_MS
#define ORBITAL_MOUSE_RESET_TIMEOUT_MS 0
#endif  // ORBITAL_MOUSE_RESET_TIMEOUT_MS
...
enum {
  ...
  /** Angle reset timeout in units of intervals. */
  RESET_TIMEOUT_INTERVALS = 
      (ORBITAL_MOUSE_RESET_TIMEOUT_MS) / (ORBITAL_MOUSE_INTERVAL_MS),
};
...
static struct {
  ...
  uint16_t timer;
  // Time when the angle should reset to up (0).
  uint16_t angle_reset_timer;
...
static void set_orbital_mouse_angle_fractional(uint16_t angle) {
  ...
  // Reset the angle reset timer when angle is manually changed
  if (ORBITAL_MOUSE_RESET_TIMEOUT_MS > 0 && (angle >> 8) != 0) {
    state.angle_reset_timer = timer_read() + ORBITAL_MOUSE_RESET_TIMEOUT_MS;
  } else {
    state.angle_reset_timer = 0;  // Disable timer if angle is 0 or timeout is disabled
  }
...
void orbital_mouse_task(void) {
  ...
  if (!state.timer || !timer_expired(now, state.timer)) {
    // Check for angle reset even if main timer is not ready
    if (state.angle_reset_timer && timer_expired(now, state.angle_reset_timer)) {
      set_orbital_mouse_angle(0);  // Reset to up
      state.angle_reset_timer = 0;
    }
    return;
  }
  ...
  if (state.move_dir) {
    ...
    // Reset the angle reset timer when moving
    if (ORBITAL_MOUSE_RESET_TIMEOUT_MS > 0 && (state.angle >> 8) != 0) {
      state.angle_reset_timer = now + ORBITAL_MOUSE_RESET_TIMEOUT_MS;
    }
  }
  ...
  if (active) {
    state.timer = (now + ORBITAL_MOUSE_INTERVAL_MS) | 1;
  } else if (state.angle_reset_timer) {
    // Keep task alive until reset timer expires
    state.timer = (now + ORBITAL_MOUSE_INTERVAL_MS) | 1;
  } else {
    state.timer = 0;
  }
